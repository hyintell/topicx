...
class CombinedTM(TopicModel):
    def __init__(self, dataset, topic_model, num_topics, embedding):
        super().__init__(dataset, topic_model, num_topics)
        print(f'Initialize CombinedTM with num_topics={num_topics}, embedding={embedding}')
        self.embedding = embedding
        self.model = CTM(num_topics=num_topics, inference_type='combined', bert_model=embedding)
